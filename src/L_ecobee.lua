...
function getPin(session)
  ...
  local data = { response_type = "ecobeePin", scope = session.scope, client_id = CID }
...
function getTokens(session)
  ...
  local data = { grant_type = "ecobeePin", code = session.auth_token, client_id = CID }
  local res = makeRequest(session, options, stringify(data))
...
local function refreshTokens(session)
  ...
  local options = { url = "/token", method = "POST",
                    headers = { Accept = "application/json", ["Content-Type"] = "application/x-www-form-urlencoded" } }
  local data = { grant_type = "refresh_token", code = session.refresh_token, client_id = CID }
  local res = makeRequest(session, options, stringify(data))
